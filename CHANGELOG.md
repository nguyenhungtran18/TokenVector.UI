...
- **L1 Wave A — widget breadth 43 → 88 constructors** (`TkvUI.Widgets` 10 widget mới + `widget_selftest` 101 → **167/167 `WIDGETS_OK`** + umbrella `widgets_breadth_selftest` **`WIDGETS_BREADTH_OK`**): thêm `CalendarWidget` (Gregorian Zeller + hit-day + dark theme), `DateTimeEditWidget` (format vi-VN + calendar popup), `ToolBoxWidget` (header accordion), `StackedWidget`, `ScrollBarWidget`, `DialWidget` (±135° arc), `CommandLinkWidget`, `KeySequenceEditWidget` (Ctrl/Alt/Shift/Meta chord), `LCDNumberWidget` (7-seg), `TourTipWidget` (anchor balloon + steps). Acceptance `WIDGETS_BREADTH_OK`: catalog **88 distinct constructors** ≥ 80 (Widgets 42 + Native 36 + Media 10), Wave A render non-blank + hit-test calendar/toolbox/scrollbar/dial/commandlink/tourtip/keyseq + native calendar + media seekbar. verify.sh +1 case `breadth`; umbrella r36. Còn lại: Wave B (Dock geometry/MDI cascade/Ribbon/PropertyGrid/TreeView multi-sel) + port Qt Tree/Dock example ≥90% — **đã xong ở entry Wave B trên**.

- **L2 wire HarfBuzz → draw path** (leaf `TkvUI.HarfBuzz` + `text_atlas_draw_shaped` + `hbwire_selftest` 14/14 `HBWIRE_OK`): tách HB FFI/orchestration khỏi `TkvUI.Bidi` thành leaf (không import Text/Bidi → phá vòng import Text↔Bidi), Text import leaf 1 chiều. `text_atlas_draw_shaped`: ASCII fast-path giữ nguyên (`text_atlas_draw_string_ex`, không regress bench 9.4ms); thieu DLL → `text_draw_fallback` cũ; có DLL → `shape_split_runs` + `shape_run_hb` per-run, gid → slot qua `glyph_gid` reverse lookup, **lazy-bake** `atlas_find_or_bake_gid` (raster glyf@scale nếu còn slot trong i18n atlas 16-capacity, không → box `vi_draw_bits(33874822719)`), advance = HB×cscale. `hb_shape_full`/`hb_shape_to_clusters` early-validate font (nbytes≥12 + sfnt version 00010000/OTTO/true) — bug: HB face-create blob rong vẫn trả .notdef → n=2 sai 0. Selftest `HBWIRE_OK`: Arabic "سلام" pixel>0 + cluster count match + RTL visual (cluster last=0), Thai/Devanagari box pixel>0, Hebrew pixel>0, ASCII path pixel>0. Bidi 194/194 + verify.sh +1 case `harfbuzz`. Đăng ký pkg.json/umbrella (r34/r35).

- **HarfBuzz shaping打通** (`TkvUI.Bidi` selftest 150 → **194/194** + `tools/fetch_harfbuzz.ps1` + verify.sh copy dll): tải official `harfbuzz-win64-14.5.0.zip` (MIT, GitHub release chính chủ) → rename `libharfbuzz-0.dll` → `libharfbuzz.dll` (+`libglib-2.0-0`, `libintl-8`). Sửa **4 bug gốc** từng làm BLOCKED (DEVELOPMENT_PLAN 2026-09-18): (1) **stride đọc struct sai** — `hb_glyph_info_t`/`hb_glyph_position_t` = **20 byte** (cluster ở +8, không phải i*8/i*16/+4 như cũ — đây là bug "glyph 2+ = 0", verify ground truth bằng ctypes); (2) **hằng số sai hoàn toàn** — `HB_DIRECTION_LTR` phải =4/RTL=5 (cũ =0/1 → `set_direction` vô nghĩa), `HB_SCRIPT_*` phải là **tag ISO 15924** (`Latn`=1281455214…) lấy đúng từ `hb_script_from_string` trên dll (cũ = số 0..31 → script rong); (3) **`hb_buffer_add_utf8` cần BYTE length** nhưng `len()` giờ là chars → encode UTF-8 tay vào R2 (`utf8_byte_len`/`utf8_encode_to_mem`, không phụ thuộc marshal `str`); (4) **`hb_language_t` là pointer interned (i64)** — truyền `str` thẳng = garbage → qua `hb_language_from_string`; `hb_version_string` trả `str` bị marshaler `CoTaskMemFree` static buffer → **heap corruption 0xC0000374** → đổi raw i64 + `read_ansi`. Thực thi: `hb_shape_full`/`hb_shape_text` (font file qua `hb_blob_create_from_file`, guess_segment_properties), bake ground-truth Arial (ABC gid 36-38 adv 1366/1479, Hello, **Ả Rập RTL joining 5 glyph clusters [8,6,4,2,0]** — khớp ctypes 100%), RTL visual order, single-char consistency, dll-vs-định nghĩa (`from_string` so constants), skip-sạch **157/157 khi thiếu dll**. Bẫy build mới gặp: gán `rc =` cho pinvoke **void** → `InvalidProgramException`; đặt tên biến `em` đụng scope selftest cũ → `len()` nhầm i64. Còn lại: wire vào draw path (Text không import Bidi — cần leaf `TkvUI.HarfBuzz` + atlas lazy-bake HB gids) — **đã xong ở entry L2 trên**.

- **H.264 decode打通 + round-trip encode→decode** (`TkvUI.H264` viết lại 8/8 `H264_OK` + `tools/h264/{OpenH264Shim,RoundTrip,ShimTest}.cs` + verify.sh +1 case): shim C# giữ struct marshal (`SDecodingParam`/`SBufferInfo`), `.tkv` chỉ gọi static methods i32/i64 + R2 buffers với R3 full-identity inline. Round-trip encoder OpenH264 → Annex B → shim decode: **11/12 frame 64×64, gray đúng cả level, RC=0** (bẫy đã thấy: `SFrameBSInfo` nhiều layer — IDR slice nằm ở `sLayerInfo[1]`, chỉ đọc layer[0] thì mất IDR → mọi frame P chết `rc=18` = `dsNoParamSets|dsRefLost`). Selftest bake stream 230B trong source: có shim/native dll → 8/8 PASS, thiếu → `H264_OK H264_SKIPPED` (verify SKIP, 0 FAIL); build KHÔNG cần dll (identity embed, resolve lười). verify.sh tự build shim từ source tracked + copy ra `OUT/` (CLR chỉ probe cạnh exe). `docs/OPENH264.md` cập nhật trạng thái. Còn lại: Main/High profile (cần FFmpeg), encode `.tkv` (vtable đã tay thử trong RoundTrip).
...
- **Nhóm B: compiler mở R2/R4/R5/R6/R9/R10/R12 → làm được** (tkvc build mới 2026-09-22 22:14, đã probe từng primitive): `TkvUI.Clipboard` 12/12 `CLIPBOARD_OK` (module mới, suite 33 module — clipboard OS thật ASCII/Việt/emoji qua GlobalAlloc + ConvertToUtf32, R1 struct vẫn đóng nên H.264/OpenH264 tiếp tục chờ); registry scan thật `fd 18/18` (R6 HKEY + R2 out-handle, M2.4 tiến 1 bước); MJPEG hot paths chuyển bitwise/reservoir/LUT/DC-only (R5, pixels bit-identical 44/44+6/6, HD vẫn ~2fps — tường là float-IDCT + per-op overhead, cần SIMD); gỡ workaround `qa_put1` (D6 sửa thật); `chr()` thay lookup 208 dòng trong SQLite (132/132, sửa luôn bug âm 128-255 → "?"); R10 `--target library`/`--subsystem` smoke OK; R12 `reserve()` chạy OK. Verify 50/0/2.
- **Nhóm A #11: +CheckBox/RadioButton/GroupBox** (`TkvUI.Widgets.Native` 164 -> 190 `NATIVE_OK`): 3 control form còn thiếu (toggle/tristate/hit-box, dot/group-exclusivity, titled-frame + content insets), render + selftest đầy đủ, LOCALS_OK. Nhóm A #13a: warm-start đo 3 lần vẫn ~1.0 s (không phải AV-cache) → lever thật là NGEN/AOT (F2), không phải chờ AV.
- **Bench M1 file-backed: thắng PyQt ~2.4×** (`tools/bench/crud_pyqt_filebench.py` mới + `docs/BENCH.md` §6.4c): cùng workload 1000 staff rows memory→file→memory — TkvUI round-trip ~81 ms (save ~78 + load ~3) vs PyQt (QSQLITE) ~195 ms (write ~187 + read ~8), đo tươi median 5 reps. Đạt tiêu chí M1 (±20%). Ghi nhận: PyQt commit có fsync, TkvUI nhờ OS flush (thiếu primitive); O(n²) serialize vẫn là bottleneck tương đối.
